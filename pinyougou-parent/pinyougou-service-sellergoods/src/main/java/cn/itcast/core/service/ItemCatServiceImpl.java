...
public class ItemCatServiceImpl implements ItemCatService {
    ...
    public List<ItemCat> findByParentId() {
       redisTemplate.delete("itemCat");
        ...
        if(itemCatList==null){
            //查询出1级商品分类的集合
            List<ItemCat> itemCatList1 = itemCatDao.findByParentId(0L);
            //遍历1级商品分类的集合
            for(ItemCat itemCat1:itemCatList1){
                //查询2级商品分类的集合(将1级商品分类的id作为条件)
                List<ItemCat> itemCatList2 = itemCatDao.findByParentId(itemCat1.getId());
                //遍历2级商品分类的集合
                for(ItemCat itemCat2:itemCatList2){
                    //查询3级商品分类的集合(将2级商品分类的父id作为条件)
                    List<ItemCat> itemCatList3 = itemCatDao.findByParentId(itemCat2.getId());
            ...
            //存入缓存
            redisTemplate.boundHashOps("itemCat").put("ItemCatList",itemCatList1);
            //存活时间
            //redisTemplate.boundHashOps("itemCat").expire(4, TimeUnit.HOURS);
            return itemCatList1;
        }
